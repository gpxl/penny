...
def _run_bd(args: list[str], cwd: str) -> str:
    ...
    try:
        ...
        if result.returncode != 0 and result.stderr:
            logger.warning("bd %s in %r (rc=%d): %s", args, cwd, result.returncode, result.stderr.strip())
        return result.stdout
    except FileNotFoundError:
        logger.warning("bd binary not found (tried %r)", _BD_BIN)
        return ""
...
def _pri_cls(priority: str) -> str:
    """Map priority string to CSS badge class."""
    return {"P1": "p1", "P2": "p2", "P3": "p3"}.get(priority, "p3")
...
class BeadsUIController(NSObject):
    ...
    def _controlAgent_(self, sender: Any) -> None:
        ...
        try:
            ...
            logger.error("BeadsUIController _controlAgent_ failed: %s", exc)
...
class Plugin(PennyPlugin):
    ...
    def setup_project(self, project_path: Path) -> bool:
        """Run `bd init --stealth` in *project_path* if .beads/ is absent."""
    ...
    def default_allowed_tools(self) -> list[str]:
        """Contribute beads CLI tool to the scoped-mode allowed list."""
        return ["Bash(bd:*)"]
    ...
    def get_tasks(self, projects: list[dict[str, Any]]) -> list[Task]:
        ...
        for project in projects:
            ...
            if not Path(path).exists():
                logger.warning("get_tasks: path not found: %s", path)
                continue
    ...
    def handle_action(self, action: str, payload: Any) -> bool:
        ...
        try:
            ...
            if r.returncode != 0:
                logger.warning("bd %s failed (rc=%d): %s", str_args, r.returncode, r.stderr.strip())
        except Exception as exc:
            logger.error("bd %s exception: %s", str_args, exc)
        return True

    def get_dashboard_cards(self, state: dict, config: dict) -> list[dict]:
        """Return the three Beads dashboard cards (tasks, agents, completed)."""
        ...
        if ready_tasks:
            rows = "".join(
                f"<tr><td>{t['task_id']}</td><td>{t['project_name']}</td>"
                f"<td><span class='badge {_pri_cls(t['priority'])}'>{t['priority']}</span></td>"
                f"<td>{t['title']}</td>"
                f"<td><button onclick=\"fetch('/api/run',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{task_id:'{t['task_id']}'}})}})\">&#9654;</button></td></tr>"
                for t in ready_tasks
            )
            task_html = (
                f"<h2>Task Queue ({len(ready_tasks)} ready)</h2>"
                f"<table><thead><tr><th>ID</th><th>Project</th><th>Pri</th><th>Title</th><th></th></tr></thead>"
                f"<tbody>{rows}</tbody></table>"
            )
        else:
            task_html = "<h2>Task Queue (0 ready)</h2><p style='color:#9ca3af;font-size:12px'>No ready tasks.</p>"
        cards.append({"name": "beads-tasks", "html": task_html})

        # Agents Running card
        if agents_running:
            rows = "".join(
                f"<tr><td>{a['task_id']}</td><td>{a.get('project_name','')}</td>"
                f"<td>{a.get('title','')}</td>"
                f"<td><span class='badge running'>&#9679; running</span></td>"
                f"<td><button onclick=\"fetch('/api/stop-agent',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{task_id:'{a['task_id']}'}})}})\">&#9632;</button></td></tr>"
                for a in agents_running
            )
            agents_html = (
                f"<h2>Agents Running ({len(agents_running)})</h2>"
                f"<table><thead><tr><th>ID</th><th>Project</th><th>Task</th><th>Status</th><th></th></tr></thead>"
                f"<tbody>{rows}</tbody></table>"
            )
        else:
            agents_html = "<h2>Agents Running (0)</h2><p style='color:#9ca3af;font-size:12px'>None running.</p>"
        cards.append({"name": "beads-agents", "html": agents_html})

        # Completed This Period card
        if completed:
            rows = "".join(
                f"<tr><td>{a['task_id']}</td><td>{a.get('project_name','')}</td>"
                f"<td>{a.get('title','')}</td>"
                f"<td><span class='badge done'>&#10003; done</span></td>"
                f"<td><button onclick=\"fetch('/api/dismiss',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{task_id:'{a['task_id']}'}})}})\">&#10005;</button></td></tr>"
                for a in completed
            )
            comp_html = (
                f"<h2>Completed This Period ({len(completed)})</h2>"
                f"<table><thead><tr><th>ID</th><th>Project</th><th>Task</th><th>Status</th><th></th></tr></thead>"
                f"<tbody>{rows}</tbody></table>"
            )
        else:
            comp_html = "<h2>Completed This Period (0)</h2><p style='color:#9ca3af;font-size:12px'>None this period.</p>"
        cards.append({"name": "beads-completed", "html": comp_html})
