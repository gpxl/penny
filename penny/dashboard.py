...
class DashboardServer:
    ...
    def shutdown(self) -> None:
        """Stop the HTTP server thread cleanly and remove the port file."""
        with self._lock:
...
let historyFilterUserSet = false;
...
function filterHistoryWith(history, periodStart, filter) {
  if (filter === 'week') {
...
function filterHistory(history, periodStart) {
  return filterHistoryWith(history, periodStart, historyFilter);
}

function setFilter(f) {
  historyFilter = f;
  historyFilterUserSet = true;
  if (lastData) {
...
function render(data) {
  ...
  if (!history.length) {
    ...
    if (!historyFilterUserSet) {
      for (const f of ['week', '4w', 'all']) {
        if (filterHistoryWith(history, pred.period_start, f).length > 0) {
          historyFilter = f;
          break;
        }
      }
    }
    histCard.innerHTML = '<h2>Session History</h2>' + renderHistory_card(history, pred.period_start);
  }
