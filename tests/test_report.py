...
from penny.report import _history_svg, _pct_color, generate_report, open_report
...
class TestPctColor:
    def test_green_below_60(self):
        assert _pct_color(0) == "#10b981"
        assert _pct_color(59.9) == "#10b981"

    def test_yellow_at_60(self):
        assert _pct_color(60) == "#eab308"

    def test_yellow_below_80(self):
        assert _pct_color(79.9) == "#eab308"

    def test_red_at_80(self):
        assert _pct_color(80) == "#ef4444"

    def test_red_at_100(self):
        assert _pct_color(100) == "#ef4444"


# ── Report reset label formatting ───────────────────────────────────────────


class TestReportResetLabels:
    @pytest.fixture
    def report_dir(self, tmp_path):
        d = tmp_path / "reports"
        with patch("penny.report.REPORT_DIR", d):
    ...
    def test_format_reset_label_called_on_both_labels(self, report_dir):
        with patch("penny.report.REPORT_DIR", report_dir), \
             patch("penny.report.format_reset_label", side_effect=lambda x: f"FMT:{x}"):
            path = generate_report(self._state_with_labels(), {}, self._mock_mgr())
        html = path.read_text()
        assert "FMT:Mar 6 at 9pm" in html
        assert "FMT:2pm" in html
...
class TestReportHeadings:
    ...
    def test_weekly_budget_heading(self, report_dir):
        with patch("penny.report.REPORT_DIR", report_dir):
            path = generate_report(self._empty_state(), {}, self._mock_mgr())
        html = path.read_text()
        assert "Weekly Budget" in html

    def test_session_budget_heading(self, report_dir):
        with patch("penny.report.REPORT_DIR", report_dir):
            path = generate_report(self._empty_state(), {}, self._mock_mgr())
        html = path.read_text()
        assert "Session Budget" in html

    def test_old_headings_removed(self, report_dir):
        with patch("penny.report.REPORT_DIR", report_dir):
            path = generate_report(self._empty_state(), {}, self._mock_mgr())
        html = path.read_text()
        assert "Current Billing Period Usage" not in html
        assert "Current Sub-Session Usage" not in html
