...
class TestPluginSetupProject:
    """Tests for Plugin.setup_project() and Plugin.default_allowed_tools()."""

    def test_default_allowed_tools_returns_bd_entry(self):
        plugin = Plugin()
        tools = plugin.default_allowed_tools()
        assert "Bash(bd:*)" in tools

    def test_setup_project_skips_when_beads_dir_exists(self, tmp_path):
        beads_dir = tmp_path / ".beads"
        beads_dir.mkdir()
        plugin = Plugin()
        result = plugin.setup_project(tmp_path)
        assert result is False

    def test_setup_project_runs_bd_init_when_missing(self, tmp_path):
        plugin = Plugin()
    ...
    def test_setup_project_returns_false_on_nonzero_exit(self, tmp_path):
        plugin = Plugin()
        with patch("subprocess.run") as mock_run:
    ...
    def test_setup_project_returns_false_when_bd_not_found(self, tmp_path):
        plugin = Plugin()
        with patch("subprocess.run", side_effect=FileNotFoundError):
            result = plugin.setup_project(tmp_path)
        assert result is False

    def test_setup_project_returns_false_on_timeout(self, tmp_path):
        plugin = Plugin()
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="bd", timeout=30)):
            result = plugin.setup_project(tmp_path)
        assert result is False
