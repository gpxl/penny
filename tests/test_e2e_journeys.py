...
class TestJourneyTaskLifecycle:
    def test_full_task_lifecycle(self, e2e_app):
        ...
        # Step 1: Verify task is in ready list
        data = _get(port, "/api/state")
        task_ids = [t["task_id"] for t in data["ready_tasks"]]
        ...
        # Step 3: Verify task moved from ready to running
        data = _get(port, "/api/state")
        ready_ids = [t["task_id"] for t in data["ready_tasks"]]
        running_ids = [a["task_id"] for a in data["state"]["agents_running"]]
        assert "task-1" not in ready_ids
...
class TestJourneyMultiTask:
    def test_spawn_multiple_then_clear_all_completed(self, e2e_app):
        ...
        assert len(data["ready_tasks"]) == 1  # only task-3 remains
...
class TestJourneyApiSnapshotShape:
    ...
    def test_snapshot_has_all_cli_required_fields(self, e2e_app):
        """The CLI's `penny tasks` parses ready_tasks; `penny agents` parses state.agents_running."""
        _, port, _ = e2e_app
        data = _get(port, "/api/state")
        ...
        assert "completed_this_period" in data
