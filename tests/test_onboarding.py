...
class TestFixMissingBeads:
    def test_runs_bd_init_for_project_without_beads(self, tmp_path):
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        config = {"projects": [{"path": str(project_dir)}]}

        with patch("penny.onboarding._init_beads", return_value=True) as mock_init:
            fixed = fix_missing_beads(config)

        mock_init.assert_called_once_with(project_dir)
        assert "myproject" in fixed

    def test_skips_project_that_already_has_beads(self, tmp_path):
        project_dir = tmp_path / "has_beads"
        project_dir.mkdir()
        (project_dir / ".beads").mkdir()
        config = {"projects": [{"path": str(project_dir)}]}

        with patch("penny.onboarding._init_beads") as mock_init:
            fixed = fix_missing_beads(config)
    ...
    def test_skips_nonexistent_path(self, tmp_path):
        config = {"projects": [{"path": str(tmp_path / "does_not_exist")}]}
    ...
    def test_returns_empty_list_on_init_failure(self, tmp_path):
        project_dir = tmp_path / "proj"
        project_dir.mkdir()
        config = {"projects": [{"path": str(project_dir)}]}

        with patch("penny.onboarding._init_beads", return_value=False):
            fixed = fix_missing_beads(config)

        assert fixed == []

    def test_handles_empty_projects(self):
        assert fix_missing_beads({}) == []
        assert fix_missing_beads({"projects": []}) == []
