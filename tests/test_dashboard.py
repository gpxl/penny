...
class TestSnapshot:
    def test_returns_expected_keys(self):
        ...
        assert "completed_this_period" in result
    ...
    def test_ready_tasks_serialized(self):
        tasks = [Task("t-1", "Title", "P1", "/tmp/p", "p")]
        app = FakeApp(ready_tasks=tasks)
        result = _snapshot(app)
    ...
    def test_completed_from_recently_completed(self):
        ...
        app = FakeApp(state=state)
        result = _snapshot(app)
        ids = [c["task_id"] for c in result["completed_this_period"]]
        assert ids == ["x", "y"]
...
class TestDashboardGET:
    ...
    def test_get_api_state_returns_json(self, dashboard_app):
        ...
        assert data["ready_tasks"][0]["task_id"] == "t-1"
    ...
    def test_get_api_state_includes_completed(self, dashboard_app):
        _, port = dashboard_app
        _, data = _get(port, "/api/state")
        completed = data["completed_this_period"]
