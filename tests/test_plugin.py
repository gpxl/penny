...
class TestPluginManagerDiscover:
    def test_discover_does_not_raise_on_empty_plugins_dir(self, tmp_path):
        mgr = PluginManager()
        with patch.object(mgr, "_plugins_dir", return_value=tmp_path):
            mgr.discover()
        assert mgr.all_plugins == {}

    def test_discover_loads_no_duplicates(self, tmp_path):
        """Double-calling discover() should not register the same plugin twice."""
        mgr = PluginManager()
        with patch.object(mgr, "_plugins_dir", return_value=tmp_path):
            mgr.discover()
            mgr.discover()
        assert all(
            list(mgr.all_plugins.keys()).count(name) == 1
            for name in mgr.all_plugins
        )
...
class TestInstallAndConfigTemplate:
    ...
    def test_config_template_beads_not_required(self):
        from pathlib import Path
        template = Path(__file__).parent.parent / "config.yaml.template"
        content = template.read_text()
        # Beads/plugins section should be commented out (inactive in current release)
        assert "# plugins:" in content or "inactive" in content
